...
- [ ] **P5.25** **A second task: Mackey–Glass regression with a causal
  ...
  **Drift**: the system parameters themselves. $\beta_t$ smoothly (sinusoidal) or
  abruptly (a switch), applied *identically to all agents* — a fleet entering a
  common new regime, which keeps a shared predictor meaningful. $\tau$ drift and
  sensor gain/bias drift are secondary channels; per-agent deviations are a
  robustness check, not the main benchmark.

  **Partition**: each agent observes its **own realisation of the same law**,
  differing in initial history and observation noise. Not one series split, and
  not $N$ noisy views of one $x_t$. This is the edge-device abstraction — many
  similar devices, each needing a local predictor, raw data staying local.

  **Task**: blockwise causal many-to-many on **non-overlapping** blocks.
  $[x_1..x_{L-1}]\to[\hat x_2..\hat x_L]$, every target one step ahead, a block
  consumed once, the boundary transition deliberately dropped (3.1% at $L=32$).
  This is the fix to the rank-1 problem, and the non-overlap is the temporal
  incest guard.

  ### ⏳ Four things the revision left open (2026-09-14)
  ...
  4. **Name the bracket.** Every agent's law is *identical*, so the optimal
     predictor is identical and there is no distribution mismatch at all. This is
     therefore **the most favourable setting for diffusion we can construct** — the
     right first distributed experiment, but one end of a bracket whose other end
     is P5.2's label skew. Say so rather than letting a reader notice that the two
  ...
  $d_{\text{model}}=16$ rather than 8, giving $p=2273$ — comparable to the
  classification model's 2908, spanning its parameter space in ~72 rounds at
  $L=32$, and leaving `diffusion_ekf_full` affordable. $L=32$, $n_b\in\{1,2,3,4\}$
  blocks per agent per round, Gaussian observations at a chosen $\sigma$.
