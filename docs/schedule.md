...
| N>10 | $N\in\{10,20,30\}$ at a common $T=500$, $n=4$, ER $p$ matched to the $N=10$ mixing gap; stationary and abrupt. Full sharing at $N=10$ by default, at $N=20,30$ only with `--full-sharing`, pending hardware ([[D114]]). `scripts/run_network_size.py` | ~16 h (to be re-estimated after the memory smoke) | runner written 2026-09-25; memory smoke first, after P5.7 |
