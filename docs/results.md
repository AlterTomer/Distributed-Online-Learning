...
batch at the receiving agent's own $\boldsymbol\theta_v^-$ — one linearisation point per
update, and no $\boldsymbol\theta_u^-$ on the wire.
...
over a run, from the ledger. It is also better *calibrated*: lower NLL in all
five cells (0.002 to 0.026), ECE equal or slightly lower. In the abrupt cell
...
Path, ring, ER $p=0.3$ and complete; 8 cells × 5 seeds, 12.6 h. The first sweep
...
ATC reaches consensus in one step and *is* centralized.
...
⚠ The sparse point was first set to ER $p=0.15$, which is below the
